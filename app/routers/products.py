...
def create_product(warehouse_id: int, p: schemas.ProductCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user_placeholder)):
...
def list_products(warehouse_id: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user_placeholder)):
